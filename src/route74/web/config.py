...
ENV_WEB_ALLOW_PUBLIC = "ROUTE74_WEB_ALLOW_PUBLIC"
ENV_WEB_WATCH_STATE_PATH = "ROUTE74_WEB_WATCH_STATE_PATH"
...
class WebConfig:
    ...
    def __post_init__(self) -> None:
        ...
        for field_name in ("db_path", "env_file", "watch_state_path"):
            if not isinstance(getattr(self, field_name), Path):
                raise ValueError(f"web {field_name} needs Path")


def parse_web_config(argv: list[str] | None = None) -> WebConfig:
    parser = argparse.ArgumentParser(prog="route74-web")
    ...
    parser.add_argument("--watch-state-path", type=Path, default=None)
    ...
    if not allow_public and not _is_loopback(host):
        raise SystemExit(f"{ENV_WEB_ALLOW_PUBLIC}=1 is required for non-loopback host {host!r}.")
    return WebConfig(
        host=host,
        port=port,
        db_path=db_path,
...
def _optional_env_value(name: str, file_env: dict[str, str]) -> str | None:
    return env_value(name, file_env, None)


def _env_port(file_env: dict[str, str]) -> int:
    value = _raw_env_value(ENV_WEB_PORT, file_env, str(DEFAULT_PORT))
    try:
        return _parse_port(value)
    except ValueError as exc:
        raise SystemExit(f"{ENV_WEB_PORT} {exc}") from exc


def _env_host(file_env: dict[str, str]) -> str:
    value = _raw_env_value(ENV_WEB_HOST, file_env, DEFAULT_HOST)
    try:
        return _parse_host(value)
    except ValueError as exc:
        raise SystemExit(f"{ENV_WEB_HOST} {exc}") from exc


def _raw_env_value(name: str, file_env: dict[str, str], default: str) -> str:
    environment_value = env_value(name, file_env, None)
